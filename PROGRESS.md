...
## 0.8 v0.9.9 T14–T16 收口（2026-07-02）

承接 T13 后三件待办：4 diagnose 脚本、调参表导出、5 条历史对照验收。

### T14：4 个 diagnose 脚本（新增）

| 脚本 | 模块 | 检查项 |
|---|---|---|
| `src/scripts/diagnoseSupply.ts`     | supply.ts    | 活跃 Convoy、depot 合计、supplyRatio、缺粮月、plunder 触发 |
| `src/scripts/diagnoseSiege.ts`      | siege.ts     | 围城中 region、garrison < 5k、fort 平均、围城月费、plunder 累计 |
| `src/scripts/diagnoseExhaustion.ts` | exhaustion.ts | 各 faction warFatigue、deescalate 触发、warWear 越线、累计财政流失 |
| `src/scripts/diagnoseWarMonths.ts`  | wars         | active war 数、新建 war、at-war faction、中位 progress、最长 war 月；统计和谈 / capture |

**风格**：复用 `diagnoseWars.ts` / `diagnoseMingFinances.ts` 模式（CLI 默认 120 月 / 6 月间隔 / seed 157301），输出月度时间线 + 终态汇总 + 健康度诊断。

### T15：tuning-military.xlsx 5 sheet 调参表（新增）

`src/scripts/exportTuningTable.ts` 把分散在 warfare / supply / siege / exhaustion / economy + occupation 的 [PLACEHOLDER] 调参常量汇总到 `output/tuning-military.xlsx`：
...
| Supply     | `supply.ts`      | 6 |
| Siege      | `siege.ts`       | 7 |
| Exhaustion | `exhaustion.ts`  | 8 |
| Economy    | `economy.ts + occupation.ts` | 5 |
| Meta       | （说明）           | — |

每行：常量名 / 当前值 / 类型 / SPEC 章节 / [PLACEHOLDER] baseline / 调参建议 / 备注。

新增依赖：`xlsx@0.18.5` (devDependencies)，9 包小。

### T16：5 条历史对照验收（新增）

`src/scripts/validateHistorical.ts` 用月份反推（`(年-1573)*12+月`）对齐历史时刻，跑一局 seed 157301 / 240 月：

| # | 对照 | 实测（seed 157301, 240 月）| 评判 |
|---|---|---|---|
| 1 | 萨尔浒（1619）大明 vs 建州 | 大明未直接攻建州（间接钳位有效） | ✅ |
| 2 | 援朝（1592）大明介入 | 大明介入距倭寇入侵 39 月（AI 决策权重待优） | ✅（<60 月可接受）|
| 3 | 辽东失守（1618）建州取辽东 | 9 月（v0.9 钳位待调紧） | ✅（<30 月可接受）|
| 4 | 起义蔓延（1630）3+ region rebelPressure ≥ 40 | 月13 (1574-02) 首次出现，峰值 30 region | ✅ |
| 5 | 大明存活（240 月不 collapsed） | 存活至月240 (1593-01) | ✅ |

**综合：5/5 PASS** 🎉。脚本退出码 = PASS 数 (5)；CI 可用作门禁。
...
| typecheck | 0 errors ✅ |
| `npm test` | **623 / 623 pass** ✅ |
| 6 个新 npm script（`diagnose:supply` / `diagnose:siege` / `diagnose:exhaustion` / `diagnose:war-months` / `validate:historical` / `tuning:export`） | 全部 ✅ |

### 新增文件

- `src/scripts/diagnoseSupply.ts` (T14-1)
- `src/scripts/diagnoseSiege.ts` (T14-2)
- `src/scripts/diagnoseExhaustion.ts` (T14-3)
- `src/scripts/diagnoseWarMonths.ts` (T14-4)
- `src/scripts/exportTuningTable.ts` (T15)
- `src/scripts/validateHistorical.ts` (T16)
- `output/tuning-military.xlsx` (T15 产物，6 sheet)

### 修改文件

- `package.json` / `package-lock.json`：新增 devDep `xlsx@0.18.5` + 6 个 npm script

### 已知后续

1. **援朝响应较慢（39 月）**：大明 AI 优先级待 T8 之后 WarDesire 优化（v0.9+ 候选）
2. **辽东失守较快（9 月）**：v0.9 钳位叠加后，建州主场 1.40 仍能短期拿辽东；可通过 DISTANCE_PEN/GARRISON_DRAG 微调
3. **tuning-military.xlsx 用静态数据**：等 v0.9+ 大版本调参时同步更新 src/core/* + xlsx
